...
{                                                   // no guarantee all given pages are mapped
  ...
  */
  ...
  while(pgit < pgnum) {
    // update pte for trang pgn + pgit
    int *fpn;
    *fpn = -1;
    MEMPHY_get_freefp(caller->mram,fpn);
    //loop qua list frame kiếm frame gán owner
    pte_set_fpn(&caller->mm->pgd[pgn + pgit], *fpn);
    // thêm vào danh sách fifo_pgn
...
int alloc_pages_range(struct pcb_t *caller, int req_pgnum, struct framephy_struct **frm_lst)
{
  ...
  */
  ...
  if (caller->mram->free_fp_list == NULL) {
    return -3000; // ko có khung khả dụng
  }
   ...
   */
    ...
    { // TODO: ERROR CODE of obtaining somes but not enough frames
      ...
      return -1; // ko đủ dung lượng giữa chừng
    }
  }

  *frm_lst = head; // gán danh sách đã cấp phát
  return 0;
}
...
int init_mm(struct mm_struct *mm, struct pcb_t *caller)
{
  ...
  vma0->vm_mm = mm;
  vma0->vm_next = NULL; // do btl ko yeu cau co vma khac
...
int print_pgtbl(struct pcb_t *caller, uint32_t start, uint32_t end)
{
  ...
  pgn_end = PAGING_PGN(cur_vma->sbrk);
